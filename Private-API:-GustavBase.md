...
Treats a string as it would be a double-quoted string in PHP. The only exception is that variables aren't resolved. But everything else like meta-characters such as `\n` or `\t` is treated as expected.
...
If you wish to use the returned value in a single quoted string, you should ensure that you set $chars to `"'\\"` because no other characters need to be escaped. If they would, the resulting string would be `'foo\"bar'` which is a literal string and therfore the backslash (`\`) would remain.  
...
Normalizes various types of linebreaks. Windows's `\r\n` and Mac OS's `\r` are changed to match Unix's `\n`.
...
For the conversion the value of `GustavBase::ENC` is used as charset and HTML5 is used as document type (as of PHP 5.4).
...
For the conversion the value of `GustavBase::ENC` is used as charset and HTML5 is used as document type (as of PHP 5.4). Invalid code unit sequences, also those invalid in the used document type, are replaced with a unicode replacement character (as of PHP 5.4).
...
Removes HTML tags of a HTML code on basis of their display and semantic meaning (if enabled), decodes HTML entities and inlines the resulting plain text. *Inlining* means that whitespaces that aren't simple spaces (such as tabs or newlines) are replaced with one.  
...
When calling this function the MB function's without specifying a value for the last parmeter, the character encoding definition, that parameter is set to the character encoding used by Gustav (`GustavBase::ENC`).  
This has the same effect as setting the MB character encoding globally using `mb_internal_encoding()`. The difference is that by doing it this way the user can still define his own global MB character encoding without getting in Gustav's way.  
...
+   The native PCRE function doesn't support capturing of offsets in multibyte strings properly. It captures the number of bytes preceding the match, rather than the number of characters. This function fixes that problem.
...
<dl>
    ...
    <dd>The path to remove the other one from. Gets passed to <code>GustavBase::path()</code>.</dd>
    
    <dt><code>$remove_path</code></dt>
    <dd>
        The path to remove from the beginning of <code>$path</code>. Gets passed to <code>GustavBase::path()</code>.<br />
        Is set to <code>null</code>, the value of <code>$_SERVER["DOCUMENT_ROOT"]</code> is used.
    </dd>
</dl>
...
Replaces directory separators in the passed path with `/` (essential for windows users) and calls `rawurlencode()` on the path segments. Moreover this function removes the document root from the beginning of the path and appends a `/` to the URL path if the path points on a directory.

<dl>
    <dt><code>$path</code></dt>
    <dd>The path to convert to an URL path. Gets passed to <code>GustavBase::path()</code>.</dd>
...
Replaces all occurences of `/` within the passed path with a directory separator (essential for windows users) and calls `rawurldecode()` on the path. Moreover the document root is prepended to the path. The returned path always has a leading directory separator.
...
If a boolean value has been passed to `$value`, `true` is returned if the option's value matches the passed one. The values `on` and `1` match `true` while `off` and `0` (sometimes `ini_get()` also returns an empty string) match `false`.  
...
Checks whether the header has already been sent back to the client and, if not, adds the specified HTTP header field to the header. Already defined header fields are overwritten.
...
For more information on superglobals see [PHP's documentation](http://php.net/manual/en/language.variables.superglobals.php).
...
For more information on variables and how valid variable names look like see [PHP documentation](http://php.net/manual/en/language.variables.basics.php).
...
<dl>
    <dt><code>$path</code></dt>
    <dd>The path of the file to read the content from. If <code>$is_url</code> is set to <code>false</code>, this value is passed to <code>GustavBase::path()</code>. If it's set to <code>true</code>, this value should be an absolute URL. If not an URL and if the specified file doesn't exist, <code>false</code> is returned.</dd>
    
    <dt><code>$is_url</code></dt>
    <dd>When setting this parameter to <code>true</code>, <code>$path</code> sould be am absolute URL. If set to <code>true</code>, the file is read using <code>file_get_contents()</code>, regardless of the value of <code>$execution_arguments</code>. In fact, that parameter is igroned when this parameter is set to <code>true</code>. If <code>allow_url_fopen<code> isn't enabled, <code>false</code> is returned.</dd>
    
    <dt><code>$execution_arguments</code></dt>
    <dd>
        Setting this parameter to a value different than <code>null</code> means that the file is executed before getting it's content. This is done by using <code>include</code> (instead of <code>file_get_contents()</code>) and does therfore work only with PHP files, otherwise no execution will happen.<br />
        If an array is passed to this parameter, each of the array's values whose key is a valid variablename is made available as a variable named like the key and can be used inside of the included file.<br />
        An <code>include<code>d should not return any value.<br />
        If <code>$is_url</code> is set to <code>true</code>, this parameter is ignored completely.
    </dd>
    
    <dt><code>$use_cache</code></dt>
    <dd>If set to <code>true</code>, the content is written into and taken from (if present) a Gustav-internal cache. Using the cache is only available when <code>$is_url</code> is set to <code>false</code> and <code>$execution_arguments</code> is set to <code>null</code>.</dd>
</dl>
...
For more information see [file_put_contents()](http://php.net/manual/en/function.file-put-contents.php).
...
<dl>
    <dt><code>$path</code></dt>
    <dd>The path of the item to remove. Gets passed to <code>GustavBase::path()</code>.</dd>
</dl>
...
<dl>
    <dt><code>$path</code></dt>
    <dd>The path of the directory to scan. Gets passed to <code>GustavBase::path()</code>.</dd>
...
<dl>
    <dt><code>$path</code></dt>
    <dd>The path of the directory to be emptied. Gets passed to <code>GustavBase::path()</code>.</dd>
</dl>
...
<dl>
    <dt><code>$path</code></dt>
    <dd>The path of the directory to create. Gets passed to <code>GustavBase::path()</code>.</dd>
</dl>
...
This function's purpose is not to split a search term entered by an user into parts, rather to split such parts into single items. To split a user-entered search term use `GustavBase::processSearchTerm()` instead.
...
Must be one of the [IANA charset names](http://www.iana.org/assignments/character-sets/character-sets.xhtml) compatible with `htmlspecialchars()`, `html_entity_decode()`, `htmlentities()` and `get_html_translation_table()`, as well as the MB functions ([supported encodings](http://php.net/manual/en/mbstring.supported-encodings.php)).
